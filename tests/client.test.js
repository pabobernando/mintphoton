...
describe("MintPhotonClient", () => {
    ...
    beforeEach(() => {
        ...
        stargate_1.SigningStargateClient.connectWithSigner.mockResolvedValue(mockStargateClient);
        client = new client_1.MintPhotonClient(config);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe("constructor", () => {
        it("should initialize with config", () => {
            expect(client).toBeInstanceOf(client_1.MintPhotonClient);
    ...
    describe("connect", () => {
        it("should connect successfully with valid signer", async () => {
            ...
            expect(stargate_1.SigningStargateClient.connectWithSigner).toHaveBeenCalledWith(config.rpcEndpoint, mockSigner, expect.objectContaining({ registry: expect.any(Object) }));
            expect(client.isConnected()).toBe(true);
            expect(client.getAddress()).toBe("atone1test123456789012345678901234567890123456");
        });
        it("should throw error when no accounts found", async () => {
            mockSigner.getAccounts.mockResolvedValue([]);
            await expect(client.connect(mockSigner)).rejects.toThrow("No wallet address found");
        });
    });
    describe("getWalletInfo", () => {
        ...
        it("should throw error when not connected", async () => {
            const disconnectedClient = new client_1.MintPhotonClient(config);
            await expect(disconnectedClient.getWalletInfo()).rejects.toThrow("Client not connected");
        });
    });
    ...
    describe("mintPhoton", () => {
        ...
        it("should handle exceptions", async () => {
            ...
            mockStargateClient.signAndBroadcast.mockRejectedValue(new Error("Network error"));
            const result = await client.mintPhoton(100);
            expect(result.success).toBe(false);
